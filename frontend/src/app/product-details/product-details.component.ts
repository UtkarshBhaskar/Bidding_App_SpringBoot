...
export class ProductDetailsComponent implements OnInit {
  ...
  isSellerNameBlur(product: any): boolean {
    ...
    if (product && product.bids) {
      ...
      console.log(isBidder);
      console.log(isSeller);
      return isBidder !== false || isSeller;
    }
    return false;
  }
  ...
  getProductDetails() {
    this.route.params.subscribe(params => {
      ...
      this.http.get<any>(environment.base_url + '/api/products/' + this.productId).subscribe(data => {
        ...
        if (new Date(this.productDetails.bids.bidClosingDate) < new Date() && this.productDetails.bids.bidStatus != 'CLOSED') {
          console.log(this.productDetails, this.highestBidder, this.highestBidAmount);
          this.finalizeBid(this.productDetails, this.highestBidder, this.highestBidAmount);
        }
      });
    });
  }

  placeBid(productDetails: any) {

    const data = {
      'bidderId': localStorage.getItem('user_id'),
      'bidId': productDetails.bids.id,
      'amount': this.yourBidAmount,
    };

    if (this.highestBidAmount == 0) {
      if (this.yourBidAmount < (productDetails.bids.bidAmount + productDetails.minBidIncrement)) {
        this.dialogService.open('Bid amount must me greater or equal to ' + (productDetails.bids.bidAmount + productDetails.minBidIncrement), environment.error_message, 'danger', environment.error);
        return;
      }
    }
